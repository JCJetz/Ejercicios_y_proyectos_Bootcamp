...
function MultipleFilters(){
    ...
    const filterChars = (name, status) => {
        // filtro por nombre
        let filteredArray = originalChars.filter(c => c.name.toLowerCase().includes(name));
        //console.log(filteredArray);
        // filtro por status
        filteredArray = 
            status === 'All' ? filteredArray : filteredArray.filter(c => c.status === status);
            //console.log(filteredArray);
            updateChars(filteredArray); // actualizo el state

        // filtro por genero
        //filteredArray = 
        //    gender === 'All' ? filteredArray : filteredArray.filter(c => c.gender === gender);
