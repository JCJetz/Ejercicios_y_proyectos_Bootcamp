...
if (!isNaN(number)) { // Asegúrate de que es un número válido
    document.write(`Tabla de multiplicar del ${number}<br>`);
    for (let i = 1; i <= 10; i++) { // Cambiar '<' por '<='
...
for (let i = 0; i < 100; i++) {
    result += Math.pow(2, i);
    if (i !== 99) { // Añade una coma y espacio si no es el último número
        result += ', ';
    }
}
...
const input = prompt("Introduce texto").toLowerCase();
